...
| [`skills/`](skills/) | Flow phiên làm việc portable: `code-triage` (chọn việc tiếp theo) → `code-start` (mở phiên) → `code-wrap` (ship + chưng cất), cộng `code-sweep` (dọn sạch mọi việc ĐÃ XONG trong một repo — hoặc chỉ một nhóm issue hay một phiên được chỉ định — chạy code-wrap từng cái) và `handbook-sync` (kéo MỘT repo lên bản handbook mới nhất, chạy từ trong repo đó). [`install.sh`](install.sh) cài chúng thành skill Claude Code — xem mục *Cài đặt máy* ngay dưới. |
