...
| [`skills/`](skills/) | Flow phiên làm việc portable: `code-triage` (chọn việc tiếp theo) → `code-start` (mở phiên) → `code-wrap` (ship + chưng cất), cộng `handbook-sync` (kéo MỘT repo lên bản handbook mới nhất, chạy từ trong repo đó). Cài thành skill Claude Code qua `install.sh`. |
